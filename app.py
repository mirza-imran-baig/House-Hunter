...
def main():
    ...
    sidebar_html = "<img  style= 'vertical-align: bottom height:100px width:100px' src='data:image/gif;base64,{}' class='img-fluid'>".format(
        img_to_bytes("hh.gif")
    )
    st.sidebar.markdown(
    ...
    activities = ["[$$-PREDICTION-$$]", "[-ABOUT-]",
                  "[-VISUALISATION-]", "[-HYDERABAD-]"]
    graphs = ["HEAT-MAP", "COUNT-PLOT",
              "DISTRIBUTION-PLOT", "SCATTER-PLOT", "BOX-PLOT"]
    ...
    if choice == "[$$-PREDICTION-$$]":
        ...
        st.header("**PARKING**")
        parking_col = st.selectbox(
            label='', options=["BOTH", "TWO_WHEELER", "FOUR_WHEELER", "NONE"])
        st.write("--------------------------------")

        st.header("**BALCONIES**")
        #balconies_col = st.selectbox(label='',options=["0","1","2","3","4","4+"])
        balconies_col = st.selectbox(
            '', options=['0', '1', '2', '3', '4', '4+'])
        st.write("--------------------------------")

        st.header("**FURNISHING**")
        furnsihing_col = st.selectbox(
            label='', options=["UN FURNISHED", "SEMI FURNISHED", "FULLY FURNISHED"])
        st.write("--------------------------------")

        st.header("**BHK:**")
        #bhk_col = st.selectbox(label='',options=["1RK","1BHK","2BHK","3BHK","4BHK","4+BHK"])
        bhk_col = st.selectbox(
            '', options=["1RK", "1BHK", "2BHK", "3BHK", "4BHK", "4+BHK"])
        st.write("--------------------------------")
        ...
        st.header("**BATHROOMS**")
        bath_col = st.selectbox(
            label='', options=["1", "2", "3", "4", "5", "5+"])
        st.write("--------------------------------")

        st.header("**WATER FACILITY**")
        water_col = st.selectbox(
            label='', options=['CORPORATION & BORE', 'BOREWELL', 'CORPORATION', 'None'])
        st.write("--------------------------------")
        ...
        furnsihing_dict = {"FULLY FURNISHED": 3,
                           'SEMI FURNISHED': 2, 'UN FURNISHED': 1}
        parking_dict = {'FOUR_WHEELER': 4,
                        'BOTH': 3, 'TWO_WHEELER': 2, 'NONE': 1}
        bhk_dict = {'4+BHK': 6, '4BHK': 5, '3BHK': 4,
                    '2BHK': 3, '1BHK': 2, '1RK': 1}
        water_dict = {'CORPORATION & BORE': 4,
                      'BOREWELL': 3, 'CORPORATION': 2, 'None': 1}
        f_tf_dict = {'4/6': 27, '5/6': 26, '2/6': 25, '4/5': 24, '1/6': 23, '1/5': 22, '2/5': 21, '3/5': 20, '5/5': 19, '3/6': 18, '6/6': 17, '1/4': 16, '2/4': 15,
                     '0/5': 14, '3/4': 13, '4/4': 12, '0/4': 11, '1/3': 10, '0/3': 9, '2/3': 8, '1/2': 7, '3/3': 6, '1/1': 5, '2/2': 4, '0/1': 3, '0/2': 2, '0/0': 1}
        maintenance_dict = {'2700': 30, '2800': 29, '2600': 28, '2500': 27, '2300': 26, '2200': 25, '2900': 24, '2100': 23, '2400': 22, '2000': 21, '1900': 20, '1600': 19, '1800': 18,
                            '1700': 17, '1500': 16, '1200': 15, '1300': 14, '1400': 13, '1000': 12, '1100': 11, '900': 10, '800': 9, '0': 8, '700': 7, '600': 6, '500': 5, '300': 4, '400': 3, '100': 2, '200': 1}
        propertySize_dict = {'1900': 19, '1700': 18, '1800': 17, '1600': 16, '1500': 15, '1400': 14, '1300': 13, '1200': 12,
                             '1100': 11, '1000': 10, '900': 9, '800': 8, '700': 7, '600': 6, '500': 5, '400': 4, '300': 3, '200': 2, '100': 1}
        locality_dict = {'BANJARAHILLS': 28, 'MADHAPUR': 27, 'KONDAPUR': 26, 'GACHIBOWLI': 25, 'MANIKONDA': 24, 'TOLICHOWKI': 23, 'MEHDIPATNAM': 22, 'SHAIKPET': 21, 'SERILINGAMPALLY': 20, 'BEGUMPET': 19, 'MIYAPUR': 18, 'PRAGATHINAGAR': 17, 'KUKATPALLY': 16,
                         'NIZAMPET': 15, 'CHANDANAGAR': 14, 'HAFEEZPET': 13, 'ATTAPUR': 12, 'SERILINGAMPALLE': 11, 'other': 10, 'YOUSUFGUDA': 9, 'NAGOLE': 8, 'AMBERPET': 7, 'BORABANDA': 6, 'UPPAL': 5, 'VANASTHALIPURAM': 4, 'RAMACHANDRAPURAM': 3, 'MALKAJGIRI': 2, 'BODUPPAL': 1}
...
################################
        ...
        acc, df, bst_model = testing_xgb([furnishing, maintanance, lift, maintananceAmt,
                                         parking, propertySize, bhk, water, balcony, bathroom, f_tf, locality])
        if st.button(' HUNT FOR HOUSES '):
            with st.spinner(text=":tophat: Our HOUSE HUNTERS are at work :tophat:"):
                ...
                st.success(
                    "Rent can vary between ₹ {} -- ₹ {} ".format(acc-2000, acc+2000))
        ...
        if graphChoice == 'HEAT-MAP':
            ...
            ''')
            choice = st.selectbox(label='', options=['balconies', 'bathroom', 'facing', 'floor', 'furnishingDesc',
                                  'gym', 'isMaintenance', 'lift', 'parking', 'property_age', 'totalFloor', 'type_bhk', 'waterSupply'])
            ...
            ''')
            choice = st.selectbox(
                label='', options=['maintenanceAmount', 'property_size', 'rent_amount'])
            ...
            ''')
            choice_1 = st.selectbox(label='First column ', options=[
                                    'maintenanceAmount', 'property_size', 'rent_amount'])
            choice_2 = st.selectbox(label='Second column ', options=[
                                    'maintenanceAmount', 'property_size', 'rent_amount'])
            ...
            st.write("---------------")
            choice_1 = st.selectbox(label='Column VS RENT_AMOUNT', options=[
                                    'balconies', 'bathroom', 'facing', 'floor', 'furnishingDesc', 'gym', 'isMaintenance', 'lift', 'parking', 'property_age', 'totalFloor', 'type_bhk', 'waterSupply'])
